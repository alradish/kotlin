    ...
    private fun IrProperty.createBackingField(
        ...
        return symbolTable.declareField(
            ...
            IrFieldImpl(
                ...
                isFinal = isFinal, isExternal = false,
        ...
        if (property.modality != Modality.ABSTRACT && (irParent !is IrClass || !irParent.isInterface)) {
            if (initializer != null || property.getter is FirDefaultPropertyGetter ||
                ...
                backingField = createBackingField(
                    property, IrDeclarationOrigin.PROPERTY_BACKING_FIELD, descriptor, type,
                    property.visibility, property.name, property.isVal, initializer?.toIrExpression()
                )
            } else if (delegate != null) {
                val delegateExpression = delegate.toIrExpression()
                backingField = createBackingField(
                    property, IrDeclarationOrigin.DELEGATE, descriptor, delegateExpression.type,
                    Visibilities.PRIVATE, Name.identifier("${property.name}\$delegate"), true, delegateExpression
                )
            }
        }
