...
class KotlinWriter(
        ...
        v?.apply {
            when (this) {
                ...
                is Node.Import -> {
                    ...
                    if (wildcard) append(".*") else if (alias != null) append(" as ").appendName(alias as String)
                }
                ...
                is Node.TypeRef.Func.Param -> {
                    if (name != null) appendName(name as String).append(": ")
                    children(type)
                }
    ...
    protected fun Node.childTypeConstraints(v: List<Node.TypeConstraint>) = this@KotlinWriter.also {
    ...
    protected fun Node.WithAnnotations.childAnns(sameLine: Boolean = false) = this@KotlinWriter.also {
    ...
    protected fun Node.WithAnnotations.childAnnsBeforeExpr(expr: Node.Expr) = this@KotlinWriter.also {
        if (anns.isNotEmpty()) {
    ...
    protected fun Node.WithModifiers.childModsBeforeType(ref: Node.TypeRef) = this@KotlinWriter.also {
        if (mods.isNotEmpty()) {
    ...
    protected inline fun Node.children(vararg v: Node?) = this@KotlinWriter.also { v.forEach { visitChildren(it) } }

    // Null list values are asterisks
    protected fun Node.bracketedChildren(v: List<Node?>, appendIfNotEmpty: String = "") = this@KotlinWriter.also {
        if (v.isNotEmpty()) {
            append('<')
    ...
    protected fun Node.childrenLines(v: Node?, extraMidLines: Int = 0, extraEndLines: Int = 0) =
        this@KotlinWriter.also { if (v != null) childrenLines(listOf(v), extraMidLines, extraEndLines) }
    ...
    protected val String.shouldEscapeIdent
        get() =
            KEYWORDS.contains(this) ||
                    all { it == '_' } ||
                    first() in '0'..'9' ||
                    any { it !in 'a'..'z' && it !in 'A'..'Z' && it !in '0'..'9' && it != '_' && it != '.'}
    ...
    companion object {
        ...
        fun <T : Appendable> write(v: Node, app: T, extrasMap: ExtrasMap? = null) =
            app.also { KotlinWriter(it, extrasMap).write(v) }
    }
}